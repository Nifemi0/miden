...
export function PlatformAdminPortal({ isWalletConnected }: PlatformAdminPortalProps) {
  ...
  const authenticatePlatformOwner = async () => {
    ...
    try {
      ...
      // Mock signature and authentication
      const mockWalletAddress = '0x1234567890abcdef1234567890abcdef12345678'
      const mockSignature = 'mock_signature_for_platform_owner'
      
      // Simulate API call to /auth/login
      const authResponse = await mockAuthLogin(mockWalletAddress, mockSignature)
          ...
          <CardContent className="text-center space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg border-l-4 border-purple-500">
              <p className="text-sm text-gray-700">
                <strong>Connected Wallet:</strong> 0x1234...5678
              </p>
            </div>
...
// Mock API functions
async function mockAuthLogin(walletAddress: string, signature: string) {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 1000))
  
  // Mock response based on wallet address
